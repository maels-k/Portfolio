...
import { Profile, Project, Skill, TimelineItem, Certification } from '@/types';
...
export default function ClientPage({ profile, projects, skills, timeline, certifications }: ClientPageProps) {
  const { t } = useI18n();
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  <div className="group flex items-center gap-4 p-4 rounded-2xl bg-slate-900/40 border border-slate-800 hover:border-cyber-cyan/30 transition-all">
                    ...
                    <div>
                      ...
                      <p className="text-white text-sm font-medium">{profile?.social_links?.email || 'ikone.mael@gmail.com'}</p>
                    </div>
                  </div>

                  <Link href={profile?.social_links?.linkedin || 'www.linkedin.com/in/koneismael'} target="_blank" className="group flex items-center gap-4 p-4 rounded-2xl bg-slate-900/40 border border-slate-800 hover:border-cyber-cyan/30 transition-all">
                    <div className="w-12 h-12 rounded-xl bg-cyber-blue/10 flex items-center justify-center text-cyber-blue group-hover:text-cyber-cyan transition-colors">
                ...
                <div className="pt-8 border-t border-slate-800/50 flex items-center gap-6">
                  <Link href={profile?.social_links?.github || 'https://github.com/maels-k'} target="_blank" className="text-gray-500 hover:text-white transition-colors">
